"""Sample 코드로, create_react_agent를 통한 데이터 수집 에이전트 생성 예제입니다."""
...
from src.agents.util import load_env_file
...
async def create_data_collector_agent(
    ...
    try:
        ...
        system_prompt = get_prompt(
            'data_collector', 'system', tool_count=len(tools)
        )
...
async def collect_data(
    ...
    try:
        ...
        if 'messages' not in result:
            logger.error(
                f"❌ result에 'messages' 키가 없습니다. result: {result}"
            )
        ...
        try:
            ...
            logger.error(
                f'❌ create_react_agent 메시지 히스토리 구성 중 오류: {e}'
            )
            full_message_history = []
