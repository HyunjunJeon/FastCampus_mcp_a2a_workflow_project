...
class SimpleCredentialService(CredentialService):
    ...
    def set_credential(
        self, security_scheme_name: str, credential: str
    ) -> None:
    ...
    async def get_credentials(
        ...
        if credential:
            logger.debug(
                f'Credential retrieved for scheme: {security_scheme_name}'
            )
        else:
            logger.debug(
                f'No credential found for scheme: {security_scheme_name}'
            )
        return credential
    ...
    def _load_credentials(self) -> None:
        ...
        for scheme_name, env_var in self.ENV_MAPPING.items():
            if os.getenv(env_var):
                loaded_count += 1
                logger.debug(
                    f'Credential loaded from {env_var} for scheme: {scheme_name}'
                )

        if loaded_count > 0:
            logger.info(
                f'Loaded {loaded_count} credentials from environment variables'
            )
        else:
...
class CompositeCredentialService(CredentialService):
    ...
    async def get_credentials(
        ...
        logger.debug(
            f'No credential found for scheme: {security_scheme_name} in any service'
        )
        return None
